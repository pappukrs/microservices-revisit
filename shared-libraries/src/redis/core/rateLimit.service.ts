...
export async function fixedWindowRateLimit(
    ...
    try {
        ...
        console.error(`[RateLimit] Fixed window error for "${key}":`, (err as Error).message);
...
export async function slidingWindowRateLimit(
    ...
    try {
        ...
        console.error(`[RateLimit] Sliding window error for "${key}":`, (err as Error).message);
        return { allowed: true, remaining: limit, limit, resetAt: now + windowSec * 1000, retryAfter: 0 };
    }
}
...
// Smooth rate limiting — allows short bursts up to bucket capacity.
// ─────────────────────────────────────────────────────────────────────────────
...
export async function tokenBucketRateLimit(
    ...
    try {
        ...
        console.error(`[RateLimit] Token bucket error for "${key}":`, (err as Error).message);
...
export function createRateLimitMiddleware(
    options: RateLimitOptions & MiddlewareOptions
) {
    const {
        strategy = "sliding",
    ...
    return async (req: any, res: any, next: any) => {
        try {
            ...
            console.error("[RateLimit] Middleware error:", (err as Error).message);
            next(); // Fail open
        }
    };
}
...
export async function getRateLimitStatus(
    ...
    try {
        ...
        console.error(`[RateLimit] Status check error for "${key}":`, (err as Error).message);
...
export async function resetRateLimit(
    ...
    try {
        ...
        console.error(`[RateLimit] Reset error for "${key}":`, (err as Error).message);
    }
}
